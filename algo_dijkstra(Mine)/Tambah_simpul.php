...
class Tambah_simpul extends Count_Bobot_Tambah_Simpul{
	...
	*/
	...
	public function dobelSimpul($nodes0, $nodes1, $index_koordinat_json, $graph){
		...
		//CALCULATE THE ORIGINAL CONTENTS (5-4), NOT THE DOBEL (4-5)
		//==============================================================		
		$index_kolom_graph = (count($graph[$nodes0])); // new index --> graph[][new_index]
		$index_kolom_graph = "";
		
		// look for index column nodes1 (4) from graph [line] [index column]
		// NUMBER OF LINES COLUMN SIMPUL
		...
		// index of graph [line] [column] to be edited
		$baris = $nodes0;
		$kolom = $index_kolom_graph;

		// take the coordinates from node 5-4
